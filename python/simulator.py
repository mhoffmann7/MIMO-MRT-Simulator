...
def singleUESetSimulation(no_ue, scenario, dpb_algorithm, assignment_method):
    pathloss, position, corr_data = getUsers(scenario,no_ue)
    pathloss, action = dpb(dpb_algorithm,assignment_method, pathloss, corr_data)
...
def multipleSimulations(no_sets, no_ue, scenario, dpb_algorithm, assignment_method, seed):
    ...
    for set in range(0,no_sets):
        bitrates, positions,cell_edges[set], actions[set] = singleUESetSimulation(no_ue, scenario, dpb_algorithm, assignment_method)
